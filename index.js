...
function enFenomenler(dizi) {
  ...
  for(let i=0; i<dizi.length; i++){
    takipci.push(dizi[i].followers); 
  }
  for(let i=0; i<takipci.length; i++){
    if(takipci[i]>100000000){
    milyon.push(dizi[i].profile)
    };
  }
  return milyon;
}
...
function platformaGoreCokGonderiYapanFenomen(dizi, platform_){
  let maxposts = 0;
  let fenomenname = "";

for(let i=0; i<dizi.length; i++){
  if (platform_ === dizi[i].platform){
    if(maxposts < dizi[i].posts){
      maxposts = dizi[i].posts;
      fenomenname = dizi[i].profile;
